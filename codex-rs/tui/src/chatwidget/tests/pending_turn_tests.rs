...
#[tokio::test]
async fn submitted_turn_shows_working_before_turn_started() {
    ...
    terminal
        ...
        .expect("draw pending turn");
    assert_chatwidget_snapshot!(
        "pending_turn_before_turn_started",
...
async fn failed_attempt_does_not_release_an_older_pending_reservation() {
    ...
    let second_attempt_reservation = chat.reserve_user_turn_pending_start();

    chat.rollback_user_turn_pending_start(second_attempt_reservation);
...
#[tokio::test]
async fn disconnect_pause_stops_working_for_a_held_pending_reservation() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;

    chat.prepare_safety_buffered_retry_submission(UserMessage::from("unacknowledged prompt"));
    assert!(chat.bottom_pane.is_task_running());
...
#[tokio::test]
async fn unavailable_thread_pause_stops_working_for_a_held_pending_reservation() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
