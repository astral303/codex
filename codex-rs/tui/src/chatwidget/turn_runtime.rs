...
impl ChatWidget {
    ...
    pub(super) fn update_task_running_state(&mut self) {
        self.bottom_pane.set_task_running(
            (self.input_queue.user_turn_pending_start && !self.input_paused)
    ...
    /// True when neither a reserved start nor a running turn still needs the status indicator.
    ///
    /// A compaction that already received `TurnStarted` has released its reservation while
    /// keeping the indicator, so rejecting an unrelated start must leave that indicator alone.
    fn status_indicator_has_no_owner(&self) -> bool {
    ...
    pub(crate) fn handle_turn_start_rejection(&mut self, message: String) {
        ...
        self.add_to_history(history_cell::new_error_event(message));
        if self.status_indicator_has_no_owner() {
            self.bottom_pane.hide_status_indicator();
        }
        if !self.input_queue.user_turn_pending_start {
            self.input_queue.submit_pending_steers_after_interrupt = false;
