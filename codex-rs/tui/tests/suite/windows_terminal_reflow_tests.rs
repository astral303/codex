...
#[path = "windows_terminal_frame_capture.rs"]
mod frame_capture;
...
const PRESENTED_FRAME_SENTINEL_COUNT: usize = 1_500;
const PRESENTED_FRAME_SENTINEL_PAYLOAD_LENGTH: usize = 80;
const PRESENTED_FRAME_RESIZE_CYCLES: usize = 3;
...
impl ReflowResponseScenario {
    fn sentinel_count(self) -> usize {
        match self {
            ...
            | Self::PresentedFrameReplay { sentinel_count, .. }
    ...
    fn expected_request_count(self) -> usize {
        match self {
            Self::FinalAnswer { .. } | Self::PresentedFrameReplay { .. } => 1,
    ...
    fn resize_capture_cycles(self) -> usize {
        match self {
            Self::PresentedFrameReplay { resize_cycles, .. } => resize_cycles,
            Self::FinalAnswer { .. }
            | Self::PreambleThenShellTool { .. }
            | Self::LongComposer { .. }
            | Self::ShellToolBatches { .. } => 0,
        }
    }
}
...
fn assert_numbered_markers_are_contiguous(text: &str, prefix: &str, count: usize) -> Result<()> {
    ...
    let marker_lines = (1..=count)
        .map(|index| {
            ...
            lines
                .iter()
                .position(|line| numbered_marker_count(line, &marker) > 0)
...
param(
    ...
    [string] $FrameObserverPath = '',

    [string] $FrameCaptureDirectory = '',

    [string] $FrameObserverReadyPath = '',

    [string] $FrameObserverStopPath = '',

    [string] $FrameObserverCompletePath = '',

    [string] $FrameObserverFailurePath = '',

    [int] $ResizeCaptureCycles = 0,
...
try {
    ...
    if ($ResizeCaptureCycles -gt 0) {
        foreach ($requiredPath in @(
            $FrameObserverPath,
            $FrameCaptureDirectory,
            $FrameObserverReadyPath,
            $FrameObserverStopPath,
            $FrameObserverCompletePath,
            $FrameObserverFailurePath
        )) {
            if ([string]::IsNullOrEmpty($requiredPath)) {
                throw 'Presented-frame capture requires every observer path.'
            }
        }
        $observerArguments = @(
            '-NoLogo',
            ...
            $FrameObserverPath,
            '-WindowHandle',
            [string] $window.Current.NativeWindowHandle,
            '-CaptureDirectory',
            $FrameCaptureDirectory,
            '-ReadyPath',
            $FrameObserverReadyPath,
        ...
        $frameObserverProcess = Start-Process `
            -FilePath $PwshPath `
            -ArgumentList $observerArguments `
            -WindowStyle Hidden `
            -PassThru

        $observerDeadline = [DateTime]::UtcNow + [TimeSpan]::FromSeconds(30)
        while (-not (Test-Path -LiteralPath $FrameObserverReadyPath)) {
            if (Test-Path -LiteralPath $FrameObserverFailurePath) {
                throw [System.IO.File]::ReadAllText($FrameObserverFailurePath)
            }
            if ($frameObserverProcess.HasExited) {
                throw "Presented-frame observer exited with code $($frameObserverProcess.ExitCode)."
            }
            if ([DateTime]::UtcNow -ge $observerDeadline) {
                throw 'Timed out waiting for the presented-frame observer.'
            }
            Start-Sleep -Milliseconds 50
        }

        for ($cycle = 1; $cycle -le $ResizeCaptureCycles; $cycle++) {
            ...
            Start-Sleep -Seconds 1
            if ($cycle -eq $ResizeCaptureCycles) {
                Save-TerminalText `
                    -Name 'after-maximize' `
                    -Text (Get-TerminalText -Terminal $terminal)
            }
        ...
        New-Item -ItemType File -Path $FrameObserverStopPath -Force | Out-Null
        $observerDeadline = [DateTime]::UtcNow + [TimeSpan]::FromSeconds(30)
        while (-not (Test-Path -LiteralPath $FrameObserverCompletePath)) {
            if (Test-Path -LiteralPath $FrameObserverFailurePath) {
                throw [System.IO.File]::ReadAllText($FrameObserverFailurePath)
            }
            if ($frameObserverProcess.HasExited) {
            ...
            if ([DateTime]::UtcNow -ge $observerDeadline) {
                throw 'Timed out stopping the presented-frame observer.'
            }
            Start-Sleep -Milliseconds 50
        }
        $frameObserverProcess.WaitForExit()
        if ($frameObserverProcess.ExitCode -ne 0) {
            throw "Presented-frame observer exited with code $($frameObserverProcess.ExitCode)."
        }
    } else {
        $windowPattern.SetWindowVisualState(
    ...
    if ($null -ne $frameObserverProcess -and -not $frameObserverProcess.HasExited) {
        try {
            New-Item -ItemType File -Path $FrameObserverStopPath -Force | Out-Null
            if (-not $frameObserverProcess.WaitForExit(10000)) {
                $frameObserverProcess.Kill()
                $frameObserverProcess.WaitForExit()
            }
        } catch {
            Write-Warning "Failed to close presented-frame observer: $_"
        }
    }
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[ignore = "requires an interactive Windows desktop and a locally built codex binary"]
async fn windows_terminal_presents_transcript_replay_as_one_frame() -> Result<()> {
    run_windows_terminal_reflow_case(
        ReflowResponseScenario::PresentedFrameReplay {
            sentinel_count: PRESENTED_FRAME_SENTINEL_COUNT,
            resize_cycles: PRESENTED_FRAME_RESIZE_CYCLES,
        },
        /*input_cycles*/ 1,
        /*simulate_mcp_startup*/ false,
    )
...
async fn run_windows_terminal_reflow_case(
    ...
    let response_mock = match response_scenario {
        ...
        ReflowResponseScenario::PresentedFrameReplay { sentinel_count, .. } => {
            responses::mount_response_once(
                &server,
                responses::sse_response(presented_frame_sentinel_response_sse(sentinel_count))
                    .set_delay(Duration::from_secs(8)),
            )
            .await
        }
    ...
    let resize_capture_cycles = response_scenario.resize_capture_cycles();
...
"#,
    ...
    let frame_observer_path = artifacts.join("capture-presented-frames.ps1");
    let frame_capture_dir = capture_dir.join("presented-frames");
    let frame_observer_ready_path = artifacts.join("frame-observer-ready.signal");
    let frame_observer_stop_path = artifacts.join("frame-observer-stop.signal");
    let frame_observer_complete_path = artifacts.join("frame-observer-complete.signal");
    let frame_observer_failure_path = artifacts.join("frame-observer.failure.txt");
    let console_process_path = artifacts.join("console-process.txt");
...
"#,
    ...
    if resize_capture_cycles > 0 {
        fs::create_dir_all(&frame_capture_dir)?;
        fs::write(
            &frame_observer_path,
            frame_capture::WINDOWS_TERMINAL_FRAME_OBSERVER,
        )?;
    }

    let mut driver = Command::new(&pwsh);
    driver
        .arg("-NoLogo")
        .arg("-NoProfile")
        .arg("-File")
    ...
    if resize_capture_cycles > 0 {
        driver
            .arg("-FrameObserverPath")
            .arg(&frame_observer_path)
            .arg("-FrameCaptureDirectory")
            .arg(&frame_capture_dir)
            .arg("-FrameObserverReadyPath")
            .arg(&frame_observer_ready_path)
            .arg("-FrameObserverStopPath")
            .arg(&frame_observer_stop_path)
            .arg("-FrameObserverCompletePath")
            .arg(&frame_observer_complete_path)
            .arg("-FrameObserverFailurePath")
            .arg(&frame_observer_failure_path)
            .arg("-ResizeCaptureCycles")
            .arg(resize_capture_cycles.to_string());
    }
    let output = driver
        .output()
    ...
    if !output.status.success() {
        ...
        let input_failure = fs::read_to_string(&input_failure_path)
            .unwrap_or_else(|_| "input helper did not report a failure".to_string());
        let frame_observer_failure = fs::read_to_string(&frame_observer_failure_path)
            .unwrap_or_else(|_| "frame observer did not report a failure".to_string());
        server.reset().await;
        bail!(
            "Windows Terminal automation failed with {}.\nstdout:\n{stdout}\nstderr:\n{stderr}\n\
             input helper:\n{input_failure}\nframe observer:\n{frame_observer_failure}\nartifacts: {}",
            output.status,
            artifacts.display()
        );
    }
    ...
    if resize_capture_cycles > 0 {
        let summary = frame_capture::analyze_presented_frames(&frame_capture_dir)?;
        eprintln!(
            "Presented-frame capture: {} frames at {:.1} ms average cadence; minimum bottom \
             occupied-row ratio {:.3}",
            summary.frame_count,
            summary.average_cadence_ms,
            summary.minimum_bottom_occupied_row_ratio,
        );
        fs::remove_dir_all(&frame_capture_dir).with_context(|| {
            format!(
                "remove successful frame captures {}",
                frame_capture_dir.display()
            )
        })?;
    }
    Ok(())
...
fn sentinel_response_sse(sentinel_count: usize) -> String {
    sentinel_response_sse_with_line_suffix(sentinel_count, "")
}

fn presented_frame_sentinel_response_sse(sentinel_count: usize) -> String {
    let line_suffix = format!(" {}", "X".repeat(PRESENTED_FRAME_SENTINEL_PAYLOAD_LENGTH));
    sentinel_response_sse_with_line_suffix(sentinel_count, &line_suffix)
}

fn sentinel_response_sse_with_line_suffix(sentinel_count: usize, line_suffix: &str) -> String {
    let text = (1..=sentinel_count)
        .map(|index| format!("- SENTINEL-{index:03}{line_suffix}"))
        .collect::<Vec<_>>()
        .join("\n");
    responses::sse(vec![
...
fn preamble_then_shell_tool_sequence(sentinel_count: usize) -> Vec<String> {
    ...
    let shell_arguments = serde_json::json!({
        "command": format!(
            "Write-Output '{TOOL_AFTER_PREAMBLE_MARKER}'; Start-Sleep -Seconds 4"
        ),
        "timeout_ms": 10_000,
    })
    .to_string();
    vec![
        responses::sse(vec![
            ...
            responses::ev_function_call("shell-tool-preamble", "shell_command", &shell_arguments),
...
fn shell_tool_response_sequence(
    ...
    for batch in 1..=batch_count {
        ...
        let mut events = Vec::with_capacity(calls_per_batch + 2);
        ...
        for call in 1..=calls_per_batch {
            ...
            events.push(responses::ev_function_call(
                &format!("shell-tool-batch-{batch}-{call}"),
                "shell_command",
                &shell_arguments,
            ));
        }
        events.push(responses::ev_completed(&response_id));
        sequence.push(responses::sse(events));
    }
...
fn unexpected_numbered_sentinel_counts(text: &str, prefix: &str, count: usize) -> Vec<String> {
    (1..=count)
        ...
        .filter_map(|sentinel| {
            let actual_count = numbered_marker_count(text, &sentinel);
...
fn numbered_marker_count(text: &str, marker: &str) -> usize {
...
#[test]
fn numbered_marker_count_requires_a_digit_boundary() {
    let text = "SENTINEL-100\nSENTINEL-1000\nSENTINEL-1009\n";

    assert_eq!(numbered_marker_count(text, "SENTINEL-100"), 1);
    assert_eq!(numbered_marker_count(text, "SENTINEL-1000"), 1);
}
